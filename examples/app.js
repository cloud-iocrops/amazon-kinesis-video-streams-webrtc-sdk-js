...
function saveCodecPreferences() {
    ...
    /** @type {string[]} */
    const selectedVideoMimeTypes = Array.from($('input[name="vcodec"]:checked'))
        .map((selectElement) => $(selectElement).val())
        .sort();

    /** @type {string[]} */
    const selectedAudioMimeTypes = Array.from($('input[name="acodec"]:checked'))
        .map((selectElement) => $(selectElement).val())
        .sort();
